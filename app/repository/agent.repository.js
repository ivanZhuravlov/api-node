...
const AgentRepository = {
    ...
    async getAgentByState(state_id) {
        try {
            const data = await db.sequelize.query("SELECT users.email, users.id FROM users INNER JOIN users_states ON users_states.user_id = users.id WHERE users_states.state_id = " + state_id + " AND users.not_assign = 0 GROUP BY users.id ORDER BY users.fname", {
                type: db.sequelize.QueryTypes.SELECT
            });
